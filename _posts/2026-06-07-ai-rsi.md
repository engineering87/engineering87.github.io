...
```
M_{n+1} = Improve(M_n, compute_n)

capability of generation n:   C_n  = C(M_n)
gain per iteration:           g_n  = C_{n+1} - C_n

explosion   if  g_n does not shrink (or grows) as n increases
plateau     if  g_n -> 0 fast enough that sum_n g_n converges  (an S-curve)
```
...
```
g_n = f( cognitive_labor , research_compute )
```
...
flowchart LR
    ...
    E -. closes the loop .-> E
    style E stroke:#b5482f,stroke-width:2px,stroke-dasharray: 5 5
</pre>
...
```
degradation per generation
  = statistical sampling error    (finite samples miss the tails)
  + expressivity error            (the model cannot represent the full distribution)
  + approximation error           (optimization does not reach the best fit)

the three interact superadditively and accumulate across generations,
and the result is generally irreversible
```
...
```
[]_T P   means   "P is provable in T"

Lob's theorem:   if   T |- ( []_T P  ->  P )   then   T |- P
```
...
<pre class="mermaid">
flowchart RL
    A2["A(n+1)<br/>parent"] -->|"must prove<br/>child stays safe"| A1["A(n)<br/>child"]
    A1 -->|"same problem<br/>one level down"| A0["A(n-1)<br/>grandchild"]
    A0 -.->|"generic self-trust<br/>blocked by Lob"| X(("?"))
    style X stroke:#b5482f,stroke-width:2px
</pre>
...
p = fraction of the work you accelerated
s = how much you accelerated it
as s -> infinity,  S -> 1 / (1 - p)
```
