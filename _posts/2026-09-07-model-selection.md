...
| | Regulatory analysis | 30,000 (10k–90k) | 1,500 (0.8k–4k) | 20% |
...
The central figures are working assumptions, not measurements, and the ranges are there on purpose: a single number would claim a precision nobody has. For documentation and reverse engineering the plausible input runs from 40,000 to 400,000 tokens depending entirely on whether you analyse one module or the whole repository, and locating yourself inside that range is the first piece of work. The cost share assumes the 5x output-to-input ratio common to the proprietary models. That assumption is load-bearing, because the share is what picks the lever. Recompute it at the 1.3x of Qwen3.8-27B and greenfield falls from 60% to 28%, test generation from 56% to 25%, requirements analysis from 50% to 21%. On an open-weight model, in other words, several clusters stop being output problems and become input problems, and the lever to pull first changes with them. The column below is the proprietary case; if you are running open weights, halve it and read again. Replace the centres with the medians from your own logs as soon as you have two weeks of them. Changing them moves the distances; it does not change the ordering.
...
![Cost per 1,000 executions by activity cluster, one dot per model](/assets/img/model-selection/01-model-map.png)

Two distances matter here, and neither is visible from a price list.

**The distance within a row is the decision.** In ticket triage the range across the models on the market runs from $0.161 to $27.50 per thousand executions, a factor of 171 on the same task. Every row has a spread of that order, between 159x and 211x. No public evidence says the expensive model classifies your tickets better. A factor like that has to be justified by a measurement, not by caution, and in my experience it usually cannot be.
...
There is also a floor effect worth naming. In triage, extraction and operator assistance the difference between all the reasonable options is a few dollars a month at normal volume. Optimising there is a waste of engineering time: choose for quality and latency instead. Real cost optimisation lives in the development family and in legacy documentation, where a bad choice gets multiplied by the volume of context.
...
It also puts a ceiling on what optimisation can return. Apply the levers that suit a cluster's profile and the saving comes out around 68% on ticket triage and 15% on greenfield development. Those two figures are my arithmetic, not a vendor's: they assume the vendors' declared rates in the chart below, a cache reuse of 70% of the input in the clusters with a stable prefix and 45% where it is partial, and batch on the clusters that can wait. What is striking is that they barely move with the model you picked. The ceiling is set by the profile, not by the price list: in greenfield the money is in the output, and caching does not touch it. In input-heavy clusters the fix is configuration; in output-heavy clusters it is changing how you work, generating in increments and calibrating effort.
...
![Published list prices per million tokens, input against output](/assets/img/model-selection/04-price-list.png)

The output-to-input ratio sits between 5x and 6x on the proprietary models almost without exception, and lower on the open-weight ones, from 1.3x on Qwen3.8-27B to 3.4x on GLM-5.3. Which means the real cost difference between two models of comparable capability comes from how many reasoning tokens they generate, not from the list price. Anthropic states that Opus 4.5 at medium effort matched Sonnet 4.5's best SWE-bench Verified score while using 76% fewer output tokens. Before changing model, change effort.
...
The same paper measures something more uncomfortable. Repeating an identical query on the same model, the thinking tokens generated vary, in their most extreme observed case by **9.7x**. That figure is the tail rather than the norm, and I would not lean on it as a typical value, but the authors' own reading of it is the part that matters: they treat it as an irreducible noise floor for any cost predictor. A model varying against itself puts a limit on what any estimate can promise, this piece included. Use these numbers to compare cost structures, not to forecast an invoice.

The caveat: Anthropic states that models from 4.7 onward use a tokenizer that produces roughly 30% more tokens for the same text, with the Sonnet 5 announcement giving a range of 1.0 to 1.35x depending on content. In the chart above that covers Sonnet 5, Opus 5 and Fable 5.1, and not Haiku 4.5. Every comparison above is per token. Per page, the Anthropic models cost more than the chart shows. If you are choosing on price, measure tokens on your own corpus first.

And prices move. Two of the models in the chart are on time-limited rates: Gemini 3.8 Flash is promotional through 31 December 2026 and doubles on 1 January 2027, and GPT-5.6 Sol is promotional at least through 21 November 2026. Gemini 3.8 Flash is currently one of the cheapest credible options in the input-heavy clusters, which means any budget resting on it doubles on New Year's Day with nothing else changing. If your business case depends on a promotional rate, that is worth knowing before you build on it.
...
## Trying cheap first, and what to measure

Two habits follow from the arithmetic, and neither is specific to a model.

The first is to try the cheap option first with an automatic check behind it, rather than choosing the expensive one up front. The check has to be genuinely automatic and genuinely cheap: tests that pass, a schema that validates, a citation that exists in the source document, a confidence above a threshold. If the verifier costs the same order of magnitude as the generator, the arrangement loses its point, and if the verifier is unreliable it moves the problem rather than solving it. When the check fails, going one step up in capability is usually enough; jumping straight to the most expensive model gives up the saving for a case that a middling model would have closed. Model cascading is an old idea and the interesting part is not the technique, it is that the rate at which you fall through tells you whether your first choice was right. If almost everything falls through, the first choice was too weak and two attempts cost more than one good one. If almost nothing does, there is a cheaper option you have not tried.

The second is to measure cost per resolved case rather than cost per call. This is also the bridge back to the chart, and it is worth being explicit about it, because the chart is in one unit and the recommendation is in another.

Cost per resolved case is roughly cost per attempt divided by the first-pass success rate, plus what the verifier costs. Which gives a rule you can apply without any of my numbers: a model that costs f times more pays for itself when it raises the first-pass success rate by more than a factor f. At a 2.7x price difference, going from 30% to 80% is break-even; going from 45% to 90% is not, and the cheap model keeps winning. That is how a row of the chart gets reordered, and it is the only way it legitimately does: not by arguing about the price list, but by measuring the two success rates on your own cases.

Cost per closed ticket, per document produced, per accepted merge. Those are the numbers that survive a model release.

## What this does not tell you

Three things.

It does not forecast a bill, and it is weaker than that in a way I got wrong in an earlier draft. I wrote that the ordering survives while the absolute figures do not. It is closer to the opposite. Every model in a row is priced on the same assumed output count, and the whole point of Chen et al. is that models differ enormously in how many thinking tokens they spend on the same task. A chart built on one output number per row therefore ranks list prices weighted by a profile, which is the thing their 32% reversal rate says you cannot do.

What survives is the wide gaps. A twentyfold distance between a small model and a frontier one does not invert because one of them thinks three times harder than assumed. Adjacent comparisons do invert, and how easily depends on the output share in the table above, which is the same idea applied to itself. Multiply the output tokens by three and the total moves by 29% in application operations, where the share is 14%, and by 120% in greenfield, where it is 60%. So read the rows at the level of tiers, not of neighbours, and trust them least exactly where the output share is highest.

It does not settle capability either. If a task genuinely needs frontier reasoning, no amount of token arithmetic makes a cheap model adequate, and the cost then becomes what the work requires rather than something to optimise. Whether an option is good enough stays a question for benchmarks, for the vendors' evaluations and, above all, for yours. Reading the two axes together is the whole point, and reading either one alone is how both failure modes start.

And it does not tell you which model to pick. That is deliberate. The chart shows what each option costs on each kind of work; the choice belongs to whoever knows the quality bar, the constraints and the volumes, and that is not me.

Which brings me to the part no article can do for you. No public benchmark measures your tickets, your regulatory texts, your repositories. A set of 100 to 200 real cases per high-volume cluster, each with an expected answer and a correctness criterion, turns every row of the token-profile table from an assumption into a measurement, and makes cost per resolved case comparable across models. It is a week of work and it outlives every model release in the next two years.

---

*Sources: the price-reversal and thinking-token variance findings come from [The Price Reversal Phenomenon: When Cheaper Reasoning Models Cost More](https://arxiv.org/abs/2603.23971), Chen, Zhang, He, Stoica, Zaharia and Zou, March 2026, revised May 2026. Prices and model line-ups as published on 6 September 2026, in US dollars, taxes excluded. Every price here was re-checked at source in a second, independent pass, which caught four errors in the first draft: DeepSeek V4 Flash is $0.0679/$0.168 and not $0.05/$0.10; Qwen3.8-27B's base output price is $0.20 and not $2.00, with a provider spread up to fifteen times; Gemini 2.5 Flash-Lite is no longer listed and was dropped; and the triage spread was misstated in an earlier draft, because I rounded the cheapest option's price before dividing. Two prices carry an expiry: Gemini 3.8 Flash is promotional through 31 December 2026 and doubles on 1 January 2027, and GPT-5.6 Sol is promotional at least through 21 November 2026. Sources: vendor documentation from [Anthropic](https://platform.claude.com/docs/en/about-claude/pricing), [OpenAI](https://developers.openai.com/api/docs/pricing) and [Google](https://ai.google.dev/gemini-api/docs/pricing); [OpenAI on retiring SWE-bench Verified](https://openai.com/index/why-we-no-longer-evaluate-swe-bench-verified/); [Artificial Analysis](https://artificialanalysis.ai/leaderboards/models) for independent measurement; [Scale AI](https://labs.scale.com/leaderboard/swe_bench_pro_public) for SWE-bench Pro under a standardised harness; OpenRouter for open-weight inference pricing. This space moves in weeks: re-check the source before spending money on the basis of any of it.*
