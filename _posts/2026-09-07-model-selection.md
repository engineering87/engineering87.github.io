...
It also puts a ceiling on what optimisation can return. Apply the levers that suit a cluster's profile and the saving comes out around 68% on ticket triage and 15% on greenfield development, and what is striking is that those two figures barely move with the model you picked. The ceiling is set by the profile, not by the price list: in greenfield the money is in the output, and caching does not touch it. In input-heavy clusters the fix is configuration; in output-heavy clusters it is changing how you work, generating in increments and calibrating effort.
...
The second is to measure cost per resolved case rather than cost per call. In development, migration and reverse engineering a more expensive model that finishes on the first attempt costs less than a cheap one that needs three, and cost per call hides exactly that. Cost per closed ticket, per document produced, per accepted merge: those are the numbers that survive a model release, and they are the only ones that let you compare an expensive model against a cheap one honestly.

## What this does not tell you

Three things.

It does not forecast a bill. The cost positions are arithmetic on published prices, exact in the way arithmetic is exact, but they run on assumed token counts and they inherit both problems measured above, so the ordering survives while the absolute figures do not. Everything around them is judgement: the thirteen clusters are a cut of the work that I find useful and that you may well draw differently, and the profiles behind them are my assumptions.
